...
On Linux, Claude Code pastes an image by running `xclip -selection clipboard -t TARGETS -o` and `xclip -selection clipboard -t image/png -o`. `--on-host` links `xclip` to `remote-claude` in a directory it puts first on the session's `PATH`; run under that name, it answers those two calls through a port forwarded back to a small responder on your Mac, which reads the clipboard with `osascript` as Claude Code does locally. Any other `xclip` call, and every call while no `rclaude` connection is open, goes to the real `xclip` if there is one. Each connection records its port in `~/.rclaude-links/<port>`, holding a token that keeps the remote's other users off the link (the directory is mode 700). The remote side tries the newest connection first and moves on from one that has ended, so several panes can be open at once, closing one breaks no other, and a Claude Code started in an earlier connection finds a later one after you reattach.
